...
export async function createPdf(
    ...
    const currentDateTime = { date: moment().format("L"), time: moment().format("LT") }
    ...
    for (let i = 0; i < numPages; i++) {
        ...
        const descriptionText =
            "No Alcohol is to be purchased using this voucher.\n" +
        ...
        // draw date value
        const dateText = currentDateTime.date
    ...
    fs.writeFileSync(`${outDir}/${voucherId}.pdf`, await voucherPdf.save())

    return {
        generatedTime: currentDateTime,
        issuer: voucher.issuer.name,
        flightNumber: voucher.flightNumber,
        outputDir: `${outDir}/${voucherId}.pdf`,
        id: voucherId
    }
}
