...
function Home() {
            ...
            .then((res: Issuer[]) => {
                if (isIssuersGetSubscribed) {
                    ...
                    for (const issuer of res) {
                        ...
                        selectItems.push({
                            label: `${issuer.name} (${issuer.initials})`,
                            value: issuer.initials
                        })
                    }
    ...
    const postVoucher = () => {
        ...
        window.api
            .addVoucher(JSON.stringify(voucher))
            .then((_) => {
            ...
            .catch((err) => {
                ...
                setShowLoadingModal(false)
                setError(
                    "Could not create the vouchers. Ensure all fields are completed correctly."
                )
                setShowErrorModal(true)
            })
    }
        ...
        <>
                ...
                <form>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
                        <label>
                            ...
                            <input
                                type="text"
                                value={voucher.flightNumber}
                                onChange={(e) =>
                                    setVoucher({ ...voucher, flightNumber: e.target.value })
                                }
                            />
                        ...
                        <label>
                            ...
                            <input
                                type="date"
                                value={voucher.date.toISOString().substring(0, 10)}
                                onChange={(e) =>
                                    setVoucher({ ...voucher, date: new Date(e.target.value) })
                                }
                            />
                            <span className="input-hint">Required</span>
                        </label>
                        <label>
                            ...
                            <div className="show-pound">
                                <input
                                    ...
                                    value={voucher.value}
                                    onChange={(e) =>
                                        setVoucher({ ...voucher, value: e.target.value })
                                    }
                                />
                            </div>
                            <span className="input-hint">Required</span>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "1rem" }}>
                        ...
                        <label style={{ width: "20%" }}>
                            ...
                            <input
                                ...
                                step="1"
                                value={voucher.count}
                                onChange={(e) => setVoucher({ ...voucher, count: +e.target.value })}
                            />
                            <span className="input-hint">Required</span>
                        </label>
                    </div>
                    <div>
                        <label style={{ width: "40%" }}>
                            ...
                            <Select
                                options={issuers}
                                value={{
                                    label: voucher.issuer.name,
                                    value: voucher.issuer.initials
                                }}
                                onChange={(e) =>
                                    setVoucher({
                                        ...voucher,
                                        issuer: { name: e?.label || "", initials: e?.value || "" }
                                    })
                                }
                            />
                            <span className="input-hint">Required</span>
                    ...
                    <div style={{ display: "flex", gap: "0.5rem", width: "100%" }}>
                        ...
                        <Button
                            ...
                            disabled={false}
                            onClick={() => setVoucher(structuredClone(emptyVoucher))}
                        >
                            Reset
                        </Button>
                    </div>
            ...
            <Modal show={showErrorModal} type={"error"}>
                <div>
                    ...
                    <div>{error || "An error occurred. Review the console for details."}</div>
            ...
            <Modal show={showSuccessModal} type={"success"}>
                <div>
                    ...
                    <div>
                        {config?.outputDir
                            ? `Your voucher has been generated and stored in ${config.outputDir}.`
