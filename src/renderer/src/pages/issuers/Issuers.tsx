...
function Issuers() {
    ...
    const postNewIssuer = () => {
        ...
        window.api.addIssuer(newIssuer).then((res: Issuer) => {
            setShowAddModal(false)
            if (res.name && res.initials) {
                setIssuers([...issuers, { name: res.name, initials: res.initials, voucher_count: 0 }])
            }
            resetNewIssuerForm();
        }).catch((err) => {
            setShowErrorModal(true);
    ...
    const removeSelected = () => {
        ...
        window.api.removeIssuer(issuersToRemove).then((_) => {
    ...
    return (<>
        ...
        <div className="container">
            <div><h1>Issuers</h1></div>
            <div className="issuer-controls">
                <Button className="primary-button issuer-control-button" onClick={() => setShowAddModal(true)}>Add Issuer</Button>
                <Button disabled={removeButtonDisabled} className="secondary-button issuer-control-button" onClick={() => setShowRemoveModal(true)}>Remove Selected</Button>
            </div>
            {issuers.length > 0 ?
                    ...
                    <table cellSpacing="0">
                        ...
                        <tbody>
                            {issuers.map((issuer: Issuer, index: number) => {
                                return (
                                    <tr key={index}>
                                        <td><input type="checkbox" onChange={() => handleCheckChange(index)} /></td>
                                        <td>{issuer.name}</td>
                ...
                :
                <div><em>No issuers added.</em></div>}
        </div>
            ...
            <div>
                ...
                <div>
                    <form style={{ display: "flex", gap: "1rem" }}>
                        ...
                        <label>
                            <span>Initials</span>
                            <input type="text" value={newIssuer.initials} onChange={(e) => setNewIssuer({ ...newIssuer, initials: e.target.value })} />
                ...
                <div style={{ marginTop: "0.5rem" }}>
                    <div style={{ display: "flex", gap: "0.5rem", justifyContent: "end", width: "100%" }}>
                        <Button className="primary-button" disabled={!newIssuer.name || !newIssuer.initials} onClick={() => {
                            postNewIssuer();
                        }}>Add</Button>
        ...
        <Modal show={showRemoveModal}>
            <div>
                ...
                <div>
                    ...
                    <ul>
                        {Object.values(checkedIssuers).map((issuer: Issuer, index: number) => {
                            return (
                                <li key={index}>{issuer.name} ({issuer.initials})</li>
                            )
                        })}
                    </ul>
                </div>
                <div style={{ marginTop: "0.5rem" }}>
                    <div style={{ display: "flex", gap: "0.5rem", justifyContent: "end", width: "100%" }}>
                        <Button className="secondary-button" onClick={() => removeSelected()}>Remove</Button>
                        <Button className="neutral-button" onClick={() => setShowRemoveModal(false)}>Cancel</Button>
                    </div>
                </div>
            </div>
        </Modal>
        <Modal show={showErrorModal} type={"error"}>
            <div>
                ...
                <div>
                    Something went wrong while trying to perform the requested action.
                </div>
                <div style={{ marginTop: "0.5rem" }}>
                    <div style={{ display: "flex", gap: "0.5rem", justifyContent: "end", width: "100%" }}>
                        <Button className="ghost-button" onClick={() => setShowErrorModal(false)}>Close</Button>
                    </div>
                </div>
            </div>
        </Modal>
